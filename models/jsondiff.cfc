component singleton {
	...
	array function diff(any first = '', any second = '', array ignoreKeys = [], boolean caseSensitive = false) {
		var diffs = [];
		if( 
			getDataType(arguments.first) != getDataType(arguments.second)
		){
			diffs.append({
				'path': [],
				'type': 'CHANGE',
			...
			for (var t = arguments.first.len() + 1; t <= arguments.second.len(); t++) {
				...
				diffs.append({
					...
					'path': [path],
					'old': '',
					'new': isNull( arguments.second[path] ) ? javaCast("null", "") : arguments.second[path]
				});
			}
			...
			for (var key in arguments.first) {
				...
				if( firstKeys.findNoCase(key) && secondKeys.findNoCase(key) && isNull( arguments.first[key]) && isNull( arguments.second[key]) ){
					keysSeen.append( key );
					continue;
				}
			...
			for (var key2 in arguments.second) {
				...
				if (secondKeys.findNoCase(key2) && !keysSeen.findNoCase(key2)) {
					if (isNull(arguments.second[key2])) {
						arguments.second[key2] = javaCast("null", "");
					}
					diffs.append({
						'type': 'ADD',
						'path': [key2],
	...
	string function getDataType(required v){
		...
		if( isStruct(arguments.v) ){
			return "STRUCT";
		}

		if( isArray(arguments.v) ){
			return "ARRAY";
		}

		if( numericCheck(arguments.v) ){
			return "NUMERIC";
		}

		if( isValid("string", arguments.v) ){
			return "STRING";
		}
